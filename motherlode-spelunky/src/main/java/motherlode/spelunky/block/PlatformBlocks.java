...
public class PlatformBlocks extends MotherlodeVariantType<Block, PlatformBlocks> {
    ...
    protected void registerBase(Identifier id) {
        Motherlode.register(
            ...
            Motherlode.id(id, name -> name + "_platform"),
            this.platform
        );

        Motherlode.register(
            Registerable.block(this.stairs, this.itemSettings),
            Motherlode.id(id, name -> name + "_platform_stairs"),
            this.stairs
        );
    }
    ...
    public void accept(ArtificeResourcePack.ClientResourcePackBuilder pack, Identifier id) {
        // Platform
        pack.addBlockState(Motherlode.id(id, name -> name + "_platform"), state -> state
            .variant("type=top", variant -> variant
                .uvlock(true)
                .model(Motherlode.id(id, name -> "block/" + name + "_platform_top")))
            .variant("type=bottom", variant -> variant
                .uvlock(true)
                .model(Motherlode.id(id, name -> "block/" + name + "_platform_bottom")))
            .variant("type=double", variant -> variant
                .uvlock(true)
                .model(Motherlode.id(id, name -> "block/" + name + "_platform_double"))
            )
        );

        pack.addBlockModel(Motherlode.id(id, name -> name + "_platform_top"), model -> model
            .parent(MotherlodeModule.id("block/templates/platform_top"))
            .texture("side", Motherlode.id(id, name -> "block/" + name + "_platform"))
            .texture("top", this.topTexture != null ? this.topTexture : Motherlode.id(id, name -> "block/" + name + "_platform_top")));

        pack.addBlockModel(Motherlode.id(id, name -> name + "_platform_bottom"), model -> model
            .parent(MotherlodeModule.id("block/templates/platform_bottom"))
            .texture("side", Motherlode.id(id, name -> "block/" + name + "_platform"))
            .texture("top", this.topTexture != null ? this.topTexture : Motherlode.id(id, name -> "block/" + name + "_platform_top")));

        pack.addBlockModel(Motherlode.id(id, name -> name + "_platform_double"), model -> model
            .parent(MotherlodeModule.id("block/templates/platform_double"))
            .texture("side", Motherlode.id(id, name -> "block/" + name + "_platform"))
            .texture("top", this.topTexture != null ? this.topTexture : Motherlode.id(id, name -> "block/" + name + "_platform_top")));

        CommonAssets.BLOCK_ITEM_FUNCTION.apply(Motherlode.id(id, name -> name + "_platform")).accept(pack, Motherlode.id(id, name -> name + "_platform_top"));

        // Platform stairs
        pack.addBlockState(Motherlode.id(id, name -> name + "_platform_stairs"), state -> state
            .variant("facing=east,shape=inner_left", variant -> variant
                .model(Motherlode.id(id, name -> "block/" + name + "_platform_stairs_inner"))
                .uvlock(true)
                .rotationY(270))
            .variant("facing=east,shape=inner_right", variant -> variant
                .model(Motherlode.id(id, name -> "block/" + name + "_platform_stairs_inner")))
            .variant("facing=east,shape=outer_left", variant -> variant
                .model(Motherlode.id(id, name -> "block/" + name + "_platform_stairs_outer"))
                .uvlock(true)
                .rotationY(270))
            .variant("facing=east,shape=outer_right", variant -> variant
                .model(Motherlode.id(id, name -> "block/" + name + "_platform_stairs_outer")))
            .variant("facing=east,shape=straight", variant -> variant
                .model(Motherlode.id(id, name -> "block/" + name + "_platform_stairs")))
        ...
        pack.addBlockModel(Motherlode.id(id, name -> name + "_platform_stairs"), model -> model
            .parent(MotherlodeModule.id("block/templates/platform_stairs"))
            .texture("side", Motherlode.id(id, name -> "block/" + name + "_platform"))
            .texture("top", this.topTexture != null ? this.topTexture : Motherlode.id(id, name -> "block/" + name + "_platform_top"))
        );
        pack.addBlockModel(Motherlode.id(id, name -> name + "_platform_stairs_inner"), model -> model
            .parent(MotherlodeModule.id("block/templates/platform_stairs_inner"))
            .texture("side", Motherlode.id(id, name -> "block/" + name + "_platform"))
            .texture("top", this.topTexture != null ? this.topTexture : Motherlode.id(id, name -> "block/" + name + "_platform_top"))
        );
        pack.addBlockModel(Motherlode.id(id, name -> name + "_platform_stairs_outer"), model -> model
            .parent(MotherlodeModule.id("block/templates/platform_stairs_outer"))
            .texture("side", Motherlode.id(id, name -> "block/" + name + "_platform"))
            .texture("top", this.topTexture != null ? this.topTexture : Motherlode.id(id, name -> "block/" + name + "_platform_top"))
        );

        CommonAssets.BLOCK_ITEM.accept(pack, Motherlode.id(id, name -> name + "_platform_stairs"));
    }

    @Override
    public void accept(ArtificeResourcePack.ServerResourcePackBuilder pack, Identifier id) {
        // Platform
        pack.addLootTable(Motherlode.id(id, name -> name + "_platform"), table -> table
            ...
            .pool(pool -> pool
                ...
                .entry(entry -> entry
                    ...
                    .name(Motherlode.id(id, name -> name + "_platform"))
                )
            )
        );

        // Platform stairs
        CommonData.DEFAULT_BLOCK_LOOT_TABLE.accept(pack, Motherlode.id(id, name -> name + "_platform_stairs"));
    }
}
