...
public class RedstoneTransmitterRenderer extends BlockEntityRenderer<RedstoneTransmitterBlockEntity> {
	...
	private Vector4f[] getGemUV(Item gemItem) {
		Sprite gemSprite = MinecraftClient.getInstance().getSpriteAtlas(SpriteAtlasTexture.BLOCK_ATLAS_TEX).apply(Motherlode.id("block/transmitter_gem"));
		if(gemItem.equals(Items.EMERALD)) return getCubeUV(gemSprite, 4);
		if(gemItem.equals(MotherlodeItems.SAPPHIRE)) return getCubeUV(gemSprite, 8);
		if(gemItem.equals(MotherlodeItems.RUBY)) return getCubeUV(gemSprite, 12);
		if(gemItem.equals(MotherlodeItems.AMETHYST)) return getCubeUV(gemSprite, 16);
		if(gemItem.equals(MotherlodeItems.TOPAZ)) return getCubeUV(gemSprite, 20);
		if(gemItem.equals(MotherlodeItems.HOWLITE)) return getCubeUV(gemSprite, 24);
		if(gemItem.equals(MotherlodeItems.ONYX)) return getCubeUV(gemSprite, 28);
