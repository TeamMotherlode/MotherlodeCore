...
public class FeaturesManagerImpl implements FeaturesManager {
    ...
    @Override
    public void addFeature(Predicate<BiomeSelectionContext> biomeSelector, GenerationStep.Feature generationStep, RegistryKey<ConfiguredFeature<?, ?>> configuredFeature) {
        FEATURES.add(new Pair<>(new Pair<>(biomeSelector, generationStep), configuredFeature));
    }

    @Override
    public RegistryKey<ConfiguredFeature<?, ?>> register(Identifier id, ConfiguredFeature<?, ?> configuredFeature) {
